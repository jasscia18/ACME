...
class ImageFeaturesDB(object):
    ...
    def get_image_feature(self, scan, viewpoint):
        ...
        print('th is is scan:', scan)  #XcA2TqTSSAj
        print('th is is viewpoint:', viewpoint)  #eb7c8095d2514ab7a84732fa41ed3594
        #print('th is is kk:', key)
        if key in self._feature_store:
            ft = self._feature_store[key]
            #print('this  is ift:', ft.shape)  #(36, 768)
        else:
            with h5py.File(self.img_ft_file, 'r') as f:
                ft = f[key][...][:, :self.image_feat_size].astype(np.float32)
                self._feature_store[key] = ft
        #         print('this else is ift:', ft.shape)  ##(36, 768)
        # print('==================================')
        return ft
                ...
                if item['included']:
                    for j,conn in enumerate(item['unobstructed']):
                        if conn and data[j]['included']:
                            ...
                            #print('this is po:', positions)  #{'fb5d8954758f4bb990c8d78d2a5d63f2': array([  7.39202, -10.6632 ,   5.08919])}
                            assert data[j]['unobstructed'][i], 'Graph should be undirected'
                            #print('this is data[j]:', data[j])
