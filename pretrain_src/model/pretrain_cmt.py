...
class GlocalTextPathCMTPreTraining(BertPreTrainedModel):
    def __init__(self, config):
        ...
        if 'sap' in config.pretrain_tasks:
            ...
            self.ConfidNet_global = nn.Sequential(
                nn.Linear(768, 768 * 2),
                nn.Linear(768 * 2, 768),
                nn.Linear(768, 1),
                nn.Sigmoid()
            )
            self.ConfidNet_local = nn.Sequential(
                nn.Linear(768, 768 * 2),
                nn.Linear(768 * 2, 768),
                nn.Linear(768, 1),
                nn.Sigmoid()
    ...
    def forward_mrc(
        ...
        if compute_loss:
            ...
            if obj_prediction_soft_labels is None:
                ...
                #print('thiis mmm:', mrc_loss)
                #mrc_loss = mrc_loss + loss_l
            return mrc_loss
        else:
    ...
    def forward_sap(
        ...
        global_confid = self.ConfidNet_global(gmap_embeds.detach())
        local_confid = self.ConfidNet_local(vp_embeds.detach())

        global_confid = torch.mean(global_confid, dim=1)
        local_confid = torch.mean(local_confid, dim=1)

        global_holo = torch.log(local_confid) / (torch.log(global_confid * local_confid) + 1e-8)
        local_holo = torch.log(global_confid) / (torch.log(global_confid * local_confid) + 1e-8)

        cb_global = global_confid + global_holo
        cb_local = local_confid + local_holo

        w_all = torch.stack((cb_global, cb_local), 1)
        softmax = nn.Softmax(1)
        w_all = softmax(w_all)
        w_global = w_all[:, 0]
        w_local = w_all[:, 1]

        global_logits = self.global_sap_head(gmap_embeds).squeeze(2) * w_global #fuse_weights
        ...
        local_logits = self.local_sap_head(vp_embeds).squeeze(2) * w_local #(1 - fuse_weights)
        ...
        if compute_loss:
            ...
            losses = global_losses + local_losses + fused_losses #+ loss_l
            return losses
        else:
